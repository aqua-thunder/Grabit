...
const CheckOut = ({ AC, handleDelete }) => {
    ...
    return (
        <>
            <div className='flex flex-col md:flex-row justify-between border border-[#e2e2e2] px-4 md:px-6 py-3 rounded-md mb-5 gap-2 md:gap-0'>
            ...
            <div className='flex flex-col lg:flex-row gap-6'>
                <div className='w-full lg:w-[70%] border border-[#dddddd] rounded-3xl px-4 py-4'>
                    {
                        AC.length === 0 ? <div className='mt-3 px-3 py-10 text-center'>
                            <span className='font-semibold text-gray-700 text-lg block mb-6'>Your cart is empty! Start shopping now and fill it with something awesome!</span>
                            <Link to="/" >
                                <button className='bg-[#5caf90] text-white font-bold px-6 py-3 rounded-md hover:bg-[#4b5966] transition-colors cursor-pointer'>Start Shopping</button>
                            </Link>
                        </div> :
                            AC.map((productItem, productInex) => {
                                return (
                                    <div key={productInex}>
                                        <div className='flex justify-between items-center mt-1 py-4'>
                                            <div className='flex items-center gap-4'>
                                                <img src={productItem.url} alt="" className='w-20 md:w-32 rounded-md object-cover border border-gray-100' />
                                            ...
                                            <div className='cursor-pointer p-2 hover:bg-gray-100 rounded-full' onClick={(e) => { { handleDelete(e, productItem.id), notify() } }}>
                ...
                <div className='border border-[#dddddd] rounded-3xl p-6 font-bold w-full lg:w-[30%] text-sm text-[#4b5966] space-y-4 h-fit'>
                    ...
                    <button className='bg-black w-full rounded-full py-3 text-white font-bold cursor-pointer mt-5 text-lg hover:bg-[#5caf90] transition-colors'  >Payment</button>
